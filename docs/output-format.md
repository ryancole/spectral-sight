...
| `has_last_hits` | bool | The creep score was read and enemy minion deaths on the world view judged against it. Needs the clock (its digits read the score) and the minion reader. When false, no row carries `cs` or `last_hits`. |
...
| `learnable` | array of strings | Present only on the `is_self` row, and only when the level-up chevrons above the ability slots were read: the slots (`"Q"`/`"W"`/`"E"`/`"R"`) whose chevron is lit, which is the set an unspent skill point could go into. Non-empty means a point is waiting; **empty means the HUD was read and shows none** — the empty array is a reading and is written, where the key's absence means nothing looked (no calibration for the row, a frame that was not the game, or the player dead). R lights only at 6, 11 and 16 and a maxed ability never does, so the set is the player's actual choice. Read by correlating each box against a captured chevron, gated by its gold, and adopted only once two consecutive readings agree — so it lags the row appearing or vanishing by about a reading. |
| `minions` | array of objects | Present only on the `is_self` row. Minions on the world view -- the player's own screen -- read from their health bars: each object is `team` (`"blue"` ours / `"red"` theirs), `x`, `y` (world-view pixels, as `threats` uses, of the minion's body, estimated 55px below its bar), `health` (bar fill fraction in [0, 1]; omitted when another bar covered this one or the frame edge cut it -- the minion is there, its health is not legible), and `world_x`, `world_y` when the world is calibrated and the camera viewport was found (projected with the champion plates' fit, so expect roughly a hundred units of error). **Empty means the view was read and shows no minions**; the key's absence means nothing looked (no calibration, or not an in-game frame). Bars in a tight clump overlap and some are lost, so this is a floor on the count. |
| `minion_dots` | array of objects | Present only on the `is_self` row. Minions anywhere on the map, from the minimap dots: `team`, `x`, `y` (minimap-crop pixels, like the row's own), `world_x`, `world_y` when calibrated. No health. Empty and absent mean what they do for `minions`. A dot under a champion marker is not seen and touching dots can merge, so counts are floors; positions are the useful part -- which lane a wave is in and how far it has pushed. |
| `turrets` | array of 22 objects | Present only on the `is_self` row, once the minimap has been read -- **always the full set of 22, never partial and never empty**; the key's absence means the minimap was not read (no calibration, not an in-game frame). Repeated on every self row while it holds, like `learnable`. Each object: `team` (`"blue"` ours / `"red"` theirs), `lane` (`"top"` / `"mid"` / `"bot"`, or `"base"` for the nexus turrets), `tier` (`"outer"` / `"inner"` / `"inhibitor"` / `"nexus"`), `standing` (bool \| null -- **null means not yet called**: the turret's spot has not been seen clearly enough either way, usually because something sits on it; the live receiver's mouse cursor covered one turret for a whole five-minute recording. Null is not "standing" and not "destroyed", and like `alive: null` it never transitions anything), and on the two nexus turrets only `side` (`"top"` / `"bot"`: which side of the nexus it guards, the one thing that tells the pair apart). The order is fixed: blue top outer, inner, inhibitor, mid ×3, bot ×3, nexus top, nexus bot, then red the same way. Read from the minimap icons, so it covers the whole map all game. A turret loss is adopted only after its spot has shown bare map -- no icon, and nothing drawn over it: no champion marker, ping, minion dot or mouse cursor -- on at least five readings spanning 5 s, so `standing` turns false about five seconds after the turret falls, and later if anything sits on the spot (a fight over the rubble, the player standing on it), since a covered spot is not read either way. This is deliberately slow: a lane turret loss is permanent, so a false one would misdescribe the map for the rest of the game. A lane turret never stands again. **A nexus turret can**: the game rebuilds them, and the client draws a destroyed one as a grey outline that turns back to colour; `standing` returns to true once the colour has held for 1.5 s. No `health`: the minimap shield carries a digit that dropped (5, 4, 2) as a turret was hit on the recorded footage, but what it counts is unverified and it is not read. |
| `cs` | int | Present only on the `is_self` row: the player's creep score, read off the score bar beside the clock. Filtered -- it never falls, a rise of one is adopted once two readings agree and a larger one once four do -- so it can lag the HUD by a reading or two. Absent when not yet read. |
| `last_hits` | array of objects | Present only on the `is_self` row, and only on the frame an entry resolved -- about 1.5s after the death, once the creep score has had its window and a briefly hidden bar its chance to return. Each object: `at` (float, `video_time` the minion's bar was last seen), `outcome` (`"last_hit"` -- the creep score rose for it; `"missed"` -- a low enemy minion died on screen and it did not; `"unknown"` -- the score was unreadable around it), `health` (the bar's last legible fill; a `last_hit` well above a third was an ability kill), `x`, `y` (world-view pixels where it died). Deaths are judged only while the player is alive; a healthy bar vanishing is credited if the score rose and otherwise ignored (it was usually hidden, not killed). |
...
| `skill_spent` | `held_for` (float, seconds; omitted if the point's arrival was never seen) | `learnable` becoming empty after being non-empty: the point went into an ability. `held_for` is how long it sat unspent, from the reading that first showed it. A missing `learnable` (nothing looked) never transitions either way, exactly as `alive: null` does not. |
...
| `last_hit` | `at`, `health`, `x`, `y` -- one entry of the row's `last_hits` whose outcome is `last_hit` | An enemy minion that died on the player's screen with their creep score rising for it. Always `is_self`. |
| `missed_cs` | `at`, `health`, `x`, `y` -- one entry of the row's `last_hits` whose outcome is `missed` | A low enemy minion that died on the player's screen without their creep score rising: someone or something else got it. An `unknown` entry makes no event. Always `is_self`. |
